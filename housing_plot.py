    ...
    for plot_t in list(range(term_t, cp.T)):
        ...
        for i_z in [1]:
            for col_ih, i_h, lbs in zip([0, 1, 2], [125, 150, 200], labs):
                pos_col = (
                    np.where(
                        np.abs(np.diff(h_pols_col_adj[plot_t, i_z, :, i_h])) > 0.09
                    )[0]
                    + 1
                )
                ...
                pos_bell = (
                    np.where(np.abs(np.diff(h_pols_adj[plot_t, i_z, :, i_h])) > 2)[0]
                    + 1
                )
                ...
                pos_col_a = (
                    np.where(
                        np.abs(np.diff(a_pols_col_adj[plot_t, i_z, :, i_h])) > 0.08
                    )[0]
                    + 1
                )
                ...
                pos_bell_a = (
                    np.where(np.abs(np.diff(a_pols_adj[plot_t, i_z, :, i_h])) > 2)[0]
                    + 1
                )
                ...
                ax_pol_a[0].plot(
                    x1a, g_1a, color=colors[col_ih], label=lbs, linewidth=0.75
                )
                ax_pol_a[1].plot(
                    x2a, g_2a, color=colors[col_ih], label=lbs, linewidth=0.75
                )
