...
# local modules
from FUES.math_funcs import rootsearch, interp_as
...
class ConsumerProblem:
    ...
    def __init__(
        ...
        self.UGgrid_all = UCGrid(
            (b, grid_max_A, grid_size), (b, grid_max_H, grid_size_H)
        )
        ...
        def y_func(t, xi):
            wage_age = np.dot(
                np.array([1, t, np.power(t, 2), np.power(t, 3), np.power(t, 4)]).astype(
                    np.float64
                ),
                lambdas[0:5],
            )
...
def Operator_Factory(cp):
    ...
    asset_grid_A, asset_grid_H, z_vals, Pi = (
        cp.asset_grid_A,
        cp.asset_grid_H,
        cp.z_vals,
        cp.Pi,
    ...
    def roots(f, a, l, h_prime, z, Ud_prime_a, Ud_prime_h, t, eps=1e-3):
        ...
        i = 0
        while True:
            x1, x2 = rootsearch(f, a, l, eps, h_prime, z, Ud_prime_a, Ud_prime_h, t)
            if np.isnan(x1) is False:
                a = x2
                root = brentq(
                    f, x1, x2, args=(h_prime, z, Ud_prime_a, Ud_prime_h, t), xtol=1e-12
                )
                if root is not None:
                    sols_array[i] = root[0]
    ...
    def obj_adj(x_prime, a, h, z, i_z, V, R, R_H, t):
        ...
        w_2 = R * a + R_H * h * (1 - delta) + y_func(t, z) - h_prime - tau * h_prime
        ...
        if w_2 > 0 and h_prime >= b:
            ...
            x_prime_nadj_star_1 = qe.optimize.nelder_mead(
                obj_noadj, x0_no_adj, bounds=bnds_nadj_2, args=args_nadj_2, tol_x=1e-10
            )[0][0]
    ...
    def bellman_operator(t, V):
        ...
        for state in range(len(X_all)):
            ...
            # obj_adjh
            w_2_max_H = (R * a + R_H * h * (1 - delta) + y_func(t, z)) / (1 + tau)

            bnds_adj = np.array([b, w_2_max_H])
            args_adj = (a, h, z, i_z, V, R, R_H, t)

            # maximise over h_prime, implicitly maximising over a_prime
            h_prime_adj_star = qe.optimize.brent_max(
                obj_adj, bnds_adj[0], bnds_adj[1], args=args_adj, xtol=1e-10
            )[0]
            ...
            a_prime_nadj_star = qe.optimize.nelder_mead(
                obj_noadj,
                x0_noadj,
                bounds=bnds_nadj,
                args=args_nadj,
            ...
            v_adj = obj_adj(h_prime_adj_star, a, h, z, i_z, V, R, R_H, t)

            v_nadj = obj_noadj_scalr(
                a_prime_nadj_star[0], w_noadj, h * (1 - delta), z, i_z, V, R, R_H, 0, t
            )
            ...
            args_nadj_1 = (w_adj_1, h_prime_adj_star, z, i_z, V, R, R_H, chi, t)
            bnds_nadj_2 = np.array([[b, w_adj_1]])
            x0_nadj_2 = np.array([w_adj_1 / 2 + b])

            a_prime_adj_star_1 = qe.optimize.nelder_mead(
                obj_noadj, x0_nadj_2, bnds_nadj_2, args=args_nadj_1
            )[0][0]
            ...
            h_prime = d_adj * h_prime_adj_star + (1 - d_adj) * R_H * h * (1 - delta)

            a_prime = d_adj * a_prime_adj_star_1 + (1 - d_adj) * a_prime_nadj_star[0]

            (
                new_h_prime[i_z, i_a, i_h],
                new_a_prime[i_z, i_a, i_h],
                new_V[i_z, i_a, i_h],
            ) = (h_prime, a_prime, v)
            new_c[i_z, i_a, i_h] = (
    ...
    def housing_euler_resid_(a_prime, h_prime, z, Ud_prime_a, Ud_prime_h, t):
        ...
        egm_a, c = root_A_liq_euler_inv(a_prime, h_prime, z, Ud_prime_a, Ud_prime_h, t)

        point = np.array([a_prime, h_prime])
        Ud_prime_h_val = eval_linear(UGgrid_all, Ud_prime_h, point, xto.LINEAR)
    ...
    def root_H_UPRIME_func(h_prime, z, Ud_prime_a, Ud_prime_h, t):
        ...
        a_prime_points = roots(
            housing_euler_resid_,
            asset_grid_A[0],
            asset_grid_A[-1],
            h_prime,
            z,
        ...
        for j in range(len(a_prime_points)):
            ...
            if a_prime_points[j] > 0:
                ...
                Ud_prime_h_val = eval_linear(
                    UGgrid_all, Ud_prime_h, point_for_c, xto.LINEAR
                )

                c = du_c_inv((Ud_prime_h_val + du_h(h_prime)) / (1 + tau))
        ...
        if len(np.where(a_prime_points == b)[0]) == 0:
            ...
            Ud_prime_h_val = eval_linear(
                UGgrid_all, Ud_prime_h, point_at_amin, xto.LINEAR
            )

            eval_linear(UGgrid_all, Ud_prime_a, point_at_amin, xto.LINEAR)
        ...
        for index_a_prime in range(len(asset_grid_A)):
            for index_h_today in range(len(asset_grid_H)):
                for index_z in range(len(z_vals)):
                    ...
                    # c = max(1e-100, c)
                    endog_grid_unrefined[index_z, index_a_prime, index_h_today] = egm_a
                    c_unrefined[index_z, index_a_prime, index_h_today] = c

                    point = np.array([a_prime, h_prime])

                    v_prime = beta * eval_linear(
                        UGgrid_all, V[index_z, :], point, xto.LINEAR
                    )
    ...
    @njit
    def refine_no_adj(endog_grid_unrefined, vf_unrefined, c_unrefined, V_prime, t):
        """
        ...
        for index_h_today in range(len(asset_grid_H)):
            for index_z in range(len(z_vals)):
                ...
                endog_grid_unrefined_points = endog_grid_unrefined[
                    index_z, :, index_h_today
                ]
                c_unrefined_points = c_unrefined[index_z, :, index_h_today]

                e_grid_cean, vf_clean, c_clean, a_prime_clean, dela = FUES(
                ...
                new_a_prime_refined[index_z, :, index_h_today] = interp_as(
                    e_grid_cean, a_prime_clean, asset_grid_A
                )
                new_c_refined[index_z, :, index_h_today] = interp_as(
                    e_grid_cean, c_clean, asset_grid_A
                )
                new_v_refined[index_z, :, index_h_today] = interp_as(
                    e_grid_cean, vf_clean, asset_grid_A
                )

                new_a_prime_refined[index_z, :, index_h_today][
                    np.where(new_a_prime_refined[index_z, :, index_h_today] < b)
                ] = b

                for i_a in range(len(asset_grid_A)):
                    if asset_grid_A[i_a] < a_under_bar:
                        new_a_prime_refined[index_z, i_a, index_h_today] = b
                        point = np.array([b, asset_grid_H[index_h_today] * (1 - delta)])
                        c = asset_grid_A[i_a] - b
                        v_at_amin = u(
                            c, asset_grid_H[index_h_today] * (1 - delta), 0
                        ) + beta * eval_linear(
                            UGgrid_all, V_prime[index_z, :], point, xto.LINEAR
                        )
    ...
    def eval_adj_pol_egm_(Ud_prime_a, Ud_prime_h, V, t):
        endog_grid_unrefined = np.zeros((len(z_vals), len(asset_grid_H), EGM_N))
        ...
        for index_h_prime in range(len(asset_grid_H)):
            for index_z in range(len(z_vals)):
                ...
                a_primes, e_grid_points = root_H_UPRIME_func(
                    h_prime,
                    z_vals[index_z],
                    Ud_prime_a[index_z, :],
                    Ud_prime_h[index_z, :],
                ...
                for i in range(len(a_primes)):
                    if a_primes[i] > 0:
                        ...
                        c_val = e_grid_points[i] - h_prime * (1 + tau) - a_primes[i]
                        v_prime = beta * eval_linear(
                            UGgrid_all, V[index_z, :], point, xto.LINEAR
                        )

                        vf_unrefined[index_z, index_h_prime, i] = (
                            u(c_val, h_prime, chi) + v_prime
                        )
    ...
    # @njit
    def refine_adj(
        endog_grid_unrefined, vf_unrefined, a_prime_unrefined, h_prime_unrefined
    ):
        ...
        for index_z in range(len(z_vals)):
            ...
            e_grid_cean, vf_clean, a_prime_clean, hprime_clean, dela = FUES(
                egrid_unref_points,
                vf_unrefined_points,
                aprime_unrefined_points,
                hprime_unrefined_points,
                b,
                m_bar=m_bar,
            )

            new_a_prime_refined[index_z, :] = interp_as(
                e_grid_cean, a_prime_clean, asset_grid_WE
            )
            new_h_prime_refined[index_z, :] = interp_as(
                e_grid_cean, hprime_clean, asset_grid_WE
            )
            new_v_refined[index_z, :] = interp_as(e_grid_cean, vf_clean, asset_grid_WE)
    ...
    def coleman_operator(t, V_prime, Ud_prime_a, Ud_prime_h):
        ...
        (
            endog_grid_unrefined_adj,
            vf_unrefined_adj,
            a_prime_unrefined_adj,
        ...
        (
            ...
            a_prime_clean,
            vf_unrefined_adj_1,
            h_prime_unrefined_adj_1,
            a_prime_unrefined_adj_1,
        ...
        for state in range(len(X_all)):
            ...
            v_nadj_val = interp_as(
                asset_grid_A, v_nadj[i_z, :, i_h], np.array([wealth_nadj])
            )[0]
            c_nadj_val = interp_as(
                asset_grid_A, c_nadj[i_z, :, i_h], np.array([wealth_nadj])
            )[0]
            a_prime_nadj_val = interp_as(
                asset_grid_A, a_prime_nadj[i_z, :, i_h], np.array([wealth_nadj])
            )[0]
            ...
            a_adj_val = interp_as(
                asset_grid_WE, a_prime_adj[i_z, :], np.array([wealth])
            )[0]
            h_adj_val = interp_as(
                asset_grid_WE, h_prime_adj[i_z, :], np.array([wealth])
            )[0]
            c_adj_val = wealth - a_adj_val - h_adj_val * (1 + tau)

            points_adj = np.array([a_adj_val, h_adj_val])

            v_adj_val = u(c_adj_val, h_adj_val, chi) + beta * eval_linear(
                UGgrid_all, V_prime[i_z], points_adj, xto.LINEAR
            )
            ...
            new_h_prime[i_z, i_a, i_h] = (
                d_adj * h_adj_val + (1 - d_adj) * h_prime_nadj_val
            )

            new_h_prime_adj[i_z, i_a, i_h] = h_adj_val

            new_a_prime[i_z, i_a, i_h] = (
                d_adj * a_adj_val + (1 - d_adj) * a_prime_nadj_val
            )
            new_a_prime_adj[i_z, i_a, i_h] = a_adj_val

            new_c[i_z, i_a, i_h] = d_adj * c_adj_val + (1 - d_adj) * c_nadj_val
            new_c_adj[i_z, i_a, i_h] = c_adj_val
            new_V_uc[i_z, i_a, i_h] = d_adj * v_adj_val + (1 - d_adj) * v_nadj_val

            new_Ud_a_uc[i_z, i_a, i_h] = beta * R * d_adj * du_c(c_adj_val) + (
                1 - d_adj
            ) * beta * R * du_c(c_nadj_val)

            # margina value of housing today if not adjusting from POV of
            # t-1

            Phi_t = du_h(h_prime_nadj_val) + eval_linear(
                UGgrid_all, Ud_prime_h[i_z], point_nadj, xto.LINEAR
            )

            new_Ud_h_uc[i_z, i_a, i_h] = (
                beta
                * R_H
                * (1 - delta)
