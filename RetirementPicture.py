...
def plot_egrids(age, e_grid, vf_work, c_worker, g_size):
    ...
    ax[0].scatter(x, vf * cp.beta - cp.delta, s=20, facecolors="none", edgecolors="r")
    ax[0].plot(
        x_clean,
    ...
    ax[1].scatter(
        np.sort(x_clean),
        np.take(x_clean - c_clean, np.argsort(x_clean)),
        s=20,
        color="blue",
        marker="x",
        linewidth=0.75,
...
def plot_cons_pol(sigma_work):
    ...
    for t, col, lab in zip(
        [17, 10, 0], ["blue", "red", "black"], ["t=18", "t=10", "t=1"]
    ):
        cons_pol = np.copy(sigma_work[t])

        # remove jump joints for plotting only
        pos = (
            np.where(np.abs(np.diff(cons_pol)) / np.diff(cp.asset_grid_A) > 0.3)[0] + 1
        )
    ...
    fig.savefig("plots/retirement/ret_cons_all.png".format())
    pl.close()
...
def plot_dcegm_cf(age, g_size, e_grid, vf_work, c_worker, a_prime, plot=True):
    ...
    np.interp(m_upper, x_clean, a_prime_clean)
    ...
    for k, c_segm in enumerate(c_segments):
        c1_env[inds_upper == k] = c_segm[
            m_segments[k].searchsorted(m_upper[inds_upper == k])
        ]

    for k, a_segm in enumerate(a_segments):
        a1_env[inds_upper == k] = np.interp(
            m_upper[inds_upper == k], m_segments[k], a_segm
        )
    ...
    if plot:
        ...
        fig.tight_layout()
        fig.savefig(
            "plots/retirement/ret_vf_aprime_all_{}_cf_{}.png".format(g_size, age)
        )
